...
front="./rococo-client/"
front_image="ev1av1ness/rococo-client:latest"
...
if [ "$1" = "push" ] || [ "$2" = "push" ]; then
  ...
  bash ./gradlew clean build dockerPush -x :rococo-e-2-e-tests:test
  cd "$front" || exit
  bash ./docker-build.sh dev push
else
  echo "### Build images (front: $front) ###"
  bash ./gradlew clean build dockerTagLatest -x :rococo-e-2-e-tests:test
  cd "$front" || exit
